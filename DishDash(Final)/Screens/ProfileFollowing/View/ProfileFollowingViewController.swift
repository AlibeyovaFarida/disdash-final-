...
//    ]
    private var viewModel = ProfileFollowingViewModel()
...
extension ProfileFollowingViewController: UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if !followingTabBottomLineImageView.isHidden{
            return viewModel.getFollowingProfileListCount()
        }
        if !followersTabBottomLineImageView.isHidden{
            return viewModel.getFollowersProfileListCount()
        }
        return 0
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if followingTabBottomLineImageView.tintColor == UIColor(named: "RedPinkMain") {
            ...
            if !self.viewModel.followingProfileList[indexPath.row].isAnimatedDone {
                cell.alpha = 0
                ...
                self.viewModel.changeFollowingProfileAnimated(at: indexPath.row)
            }
            cell.configure(viewModel.followingProfileList[indexPath.row])
            return cell
        }
        if followersTabBottomLineImageView.tintColor == UIColor(named: "RedPinkMain"){
            ...
            if !self.viewModel.followersProfileList[indexPath.row].isAnimatedDone {
                cell.alpha = 0
                cell.transform = CGAffineTransform(translationX: cell.contentView.frame.width, y:cell.contentView.frame.height)
                UIView.animate(withDuration: 0.3, delay: 0.3*Double(indexPath.row), animations: {
                        cell.transform = CGAffineTransform(translationX: cell.contentView.frame.width, y: cell.contentView.frame.height)
                ...
                self.viewModel.changeFollowersProfileAnimated(at: indexPath.row)
            }
            cell.configure(self.viewModel.followersProfileList[indexPath.row])
            return cell
        }
